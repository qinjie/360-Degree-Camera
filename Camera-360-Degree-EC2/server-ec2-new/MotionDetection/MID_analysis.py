...
def analysis_two_pano(pano1_path, pano2_path, cam_alsd_path):
    im1 = cv2.imread(pano1_path)
    im2 = cv2.imread(pano2_path)
    
    im1_clone = cv2.resize(im1, (320, 240))
    im2_clone = cv2.resize(im2, (320, 240))
    addImg(im1_clone, True)
    result = addImg(im2_clone, False)
    print("Store Image analysised")
    cv2.imwrite(cam_alsd_path, im2_clone)
    return result
